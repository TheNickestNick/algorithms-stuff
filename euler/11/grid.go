...
func product4(r int, c int, rd int, cd int) int {
	prod := 1
...
func maxInDirection(rd int, cd int) int {
	max := 0
	for r, row := range grid {
		for c, _ := range row {
			max = maxi(product4(r, c, rd, cd), max)
		}
	}
	return max
}
...
func main() {
	initGrid()
	max := maxInDirection(0, 1)
	max = maxi(maxInDirection(1, 0), max)
	max = maxi(maxInDirection(1, 1), max)
	max = maxi(maxInDirection(-1, 1), max)
	fmt.Println(max)
}